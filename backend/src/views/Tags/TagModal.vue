                        ...
                        <DialogPanel class="relative bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:max-w-[700px] sm:w-full">
                            ...
                            <header class="py-3 px-4 flex justify-between items-center">
                                <DialogTitle as="h3" class="text-lg leading-6 font-medium text-gray-900">
                                    {{ tag.id ? `Update tag: "${props.tag.name}"` : 'Create new tag' }}
                                </DialogTitle>
                                <button 
                                    @click="closeModal()"
                            ...
                            <form @submit.prevent="onSubmit">
                                <div class="bg-white px-4 pt-5 pb-4">
                                    <CustomInput class="mb-2" v-model="tag.name" label="Name" :errors="errors['name']"/>
                                    ...
                                    <div class="flex items-center gap-4">
                                        <div class="my-4">
                                            <img v-if="tag.image_url" class="w-16 h-16" :src="tag.image_url" :alt="tag.name">
                                            <img v-else class="w-16 h-16 object-cover" src="../../assets/noimage.png">
                                        </div>
                                        <CustomInput type="file" class="mb-2" label="tag Image" @change="file => tag.image = file"/>
                                    </div>
                                    <CustomInput type="checkbox" class="mb-2" v-model="tag.active" label="Active"  :errors="errors['active']"/>
                                </div>
...
const parentTags = computed(() => {
    return [
        {key: '', text: 'Select Parent Tag'},
        ...store.state.tags.data
        .filter(c => {
            if (tag.value.id) {
                return c.id !== tag.value.id
            }
            return true;
        })
    ...
    if (tag.value.id) {
        ...
        .then(response => {
            ...
            if (response.status === 200) {
                store.commit('showToast', 'Tag was successfully updated');
        ...
        store.dispatch('createTag', tag.value)
        .then(response => {
            loading.value = false;
            if (response.status === 201) {
                store.commit('showToast', 'Tag was successfully created');
                store.dispatch('getTags')
                closeModal()
            }
        })
