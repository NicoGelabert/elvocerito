    ...
    <div class="bg-white p-4 rounded-lg shadow animate-fade-in-down">
        <div class="flex flex-col md:flex-row justify-between border-b-2 pb-3 gap-4">
            <div class="flex flex-col md:flex-row gap-4 md:items-center">
                ...
                <span class="ml-3">Encontradas {{categories.total}} categorías</span>
            </div>
            <div>
                <input v-model="search" @input="getCategories(null)"
        ...
        <table class="table-auto w-full text-sm">
            <thead class="hidden md:contents">
                ...
                class=""
                :class="sortField === 'id' ? 'flex justify-between' : 'text-center'">
                ID
                </TableHeaderCell>
                ...
                :class="sortField === 'name' ? 'flex gap-4 justify-start' : 'text-left'">
                Nombre
                </TableHeaderCell>
                <TableHeaderCell field="active"
                    :sort-field="sortField"
                    :sort-direction="sortDirection"
                    @click="sortProducts('active')" class="text-center"
                    :class="sortField === 'active' ? 'flex justify-around' : 'text-center'">
                    Activo
                </TableHeaderCell>
                <TableHeaderCell field="updated_at"
                :sort-field="sortField"
                :sort-direction="sortDirection"
                @click="sortCategories('updated_at')"
                :class="sortField === 'updated_at' ? 'flex gap-4 justify-start' : 'text-left'">
                Última actualización
                </TableHeaderCell>
                <TableHeaderCell field="actions" class="text-center">
                Acciones
                </TableHeaderCell>
                </tr>
            </thead>
            ...
            <tbody v-else>
                <tr v-for="(category, index) of categories.data" :key="index">
                    <td class="border-b p-2 text-center w-16">{{ category.id }}</td>
                    <td class="border-b p-2 md:p-2 hidden md:table-cell w-24">
                        <img class="w-10 h-10 object-cover mx-auto" :src="category.image_url" :alt="category.name">
                    </td>
                    ...
                    <td class="border-b p-2 hidden md:table-cell text-center">
                        ...
                        <span
                            :class="{
                            'w-2 h-2 rounded-full bg-green-500': category.active,
                            'w-2 h-2 rounded-full bg-red-500': !category.active
                            }"
                        ></span>
        ...
        <div v-if="!categories.loading" class="flex flex-col md:flex-row gap-4 justify-between items-center mt-5">
        <div v-if="categories.data.length">
        Mostrando desde {{ categories.from }} a {{ categories.to }}
        </div>
        <nav
