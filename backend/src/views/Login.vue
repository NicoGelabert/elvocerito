    ...
    <guest-layout title="Sign in to your account">
        ...
        <form class="mt-8 space-y-6 w-[350px]" method="POST" @submit.prevent="login">
            ...
            <div class="flex items-center justify-between">
                ...
                <div class="text-sm">
                <router-link :to="{name: 'requestPassword'}" class="font-medium text-black hover:text-black/50"> Forgot
                    your password?
                </router-link>
                </div>
            </div>
            ...
            <div>
                <button type="submit"
                        :disabled="loading"
                        class="group relative w-full flex justify-center py-2 px-4 border border-transparent text-sm font-medium rounded-md text-white bg-black hover:bg-black/10 hover:text-black/50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-black/50"
                        :class="{
                        'cursor-not-allowed': loading,
                        'hover:bg-black/50': loading,
                        }">
                <svg
                    v-if="loading"
